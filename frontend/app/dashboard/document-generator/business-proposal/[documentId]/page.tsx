...
export default function BusinessProposalDetailsPage() {
  ...
  const [document, setDocument] = useState<BusinessProposalResponse | null>(
    null
  );
  ...
  // Fetch business proposal data
  useEffect(() => {
    const fetchProposal = async () => {
      if (!documentId) return;
      setLoading(true);
      try {
        const data = await getBusinessProposalById(Number.parseInt(documentId));
        setDocument(data);
      } catch (err: any) {
        console.error("Failed to fetch business proposal:", err);
        setError(
          err.response?.data?.message ||
            err.message ||
            "Failed to load business proposal"
        );
        toast.error("Failed to load business proposal");
      } finally {
        setLoading(false);
      }
    };
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${document?.project_title || "business-proposal"}.pdf`);
    } catch (error) {
  ...
  const downloadAsDOCX = async () => {
    ...
    try {
        ...
        sections: [
          {
            ...
            children: [
              new Paragraph({
                text: document.project_title || "Business Proposal",
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Client: ${document.client_name}`,
                    bold: true,
                  }),
                ],
              }),
      ...
      link.href = url;
      link.download = `${document?.project_title || "business-proposal"}.docx`;
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteBusinessProposal(document.id);
      ...
      toast.error(
        error.response?.data?.message ||
          error.message ||
          "Failed to delete business proposal"
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  const handleSave = async () => {
    ...
    setIsUpdating(true);
    try {
      const updatedDocument = await updateBusinessProposal(document.id, {
        ai_generated_content: editedContent,
      });
      setDocument(updatedDocument);
      setIsEditing(false);
      toast.success("Business proposal updated successfully!");
    } catch (error: any) {
      console.error("Failed to update business proposal:", error);
      toast.error(
        error.response?.data?.message ||
          error.message ||
          "Failed to update business proposal"
      );
    } finally {
      setIsUpdating(false);
    }
  ...
  if (loading) {
    return (
      <div className="w-full">
        <DesktopHeader />
        <MobileHeader className="mb-2" showBorder title="Business Proposal" />
        ...
        <div className="flex min-h-[calc(100svh-72px)] items-center justify-center lg:min-h-[calc(100svh-56px)]">
          <div className="text-center">
            <div className="mb-4 text-red-500">
              {error || "Business proposal not found"}
            </div>
