...
export default function TermsOfServiceDetailsPage() {
  const params = useParams();
  const router = useRouter();
  const documentId = params.documentId as string;

  const [document, setDocument] = useState<TermsOfServiceResponse | null>(null);
  ...
  useEffect(() => {
    const fetchTos = async () => {
      ...
      setLoading(true);
      try {
        const data = await getTermsOfServiceById(Number.parseInt(documentId));
        setDocument(data);
      } catch (err: any) {
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      const name = document?.company_name
        ? `${document.company_name}-terms-of-service`
        : "terms-of-service";
      pdf.save(`${name}.pdf`);
    } catch (error) {
        ...
        sections: [
          {
            ...
            children: [
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Company: ${document.company_name}`,
                    bold: true,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Website: ${document.website_url}`,
                    bold: true,
                  }),
                ],
              }),
  ...
  const handleDelete = async () => {
    if (!document) return;
    setIsDeleting(true);
    // setIsDeleting handles loading state
    try {
      await deleteTermsOfService(document.id);
  ...
  const handleSave = async () => {
    if (!document) return;
    setIsUpdating(true);
    // setIsUpdating handles loading state
    try {
      const updated = await updateTermsOfService(document.id, {
  ...
  if (loading) {
    return (
      <div className="w-full">
        <DesktopHeader />
        <MobileHeader className="mb-2" showBorder title="Terms of Service" />
      ...
      <div className="w-full">
        ...
        <div className="flex min-h-[calc(100svh-72px)] items-center justify-center lg:min-h-[calc(100svh-56px)]">
          <div className="text-center">
            <div className="mb-4 text-red-500">{error || "Not found"}</div>
    ...
    <div className="w-full">
      ...
      <div className="relative flex min-h-[84vh] w-full lg:min-h-[83vh]">
        ...
        <div className="-translate-y-1/2 fixed top-1/2 right-3 z-10 flex transform flex-col gap-3 sm:right-6 sm:gap-4 lg:right-8">
          <Button
            className={`h-16 w-16 rounded-full shadow-lg ${isEditing ? "border-green-300 bg-green-50 text-green-600 hover:bg-green-100 hover:text-green-600" : "border-purple-300 bg-purple-50 text-purple-600 hover:bg-purple-100 hover:text-purple-600"}`}
            disabled={isUpdating}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to delete this Terms of Service? This
