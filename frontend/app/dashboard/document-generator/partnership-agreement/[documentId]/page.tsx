...
const BetterEditor = dynamic(() => import("@/components/better-editor"), { ssr: false });
...
export default function PartnershipAgreementDetailsPage() {
  ...
  const [document, setDocument] = useState<PartnershipAgreementResponse | null>(null);
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      ...
      setLoading(true);
      try {
        const data = await getPartnershipAgreementById(parseInt(documentId));
        setDocument(data);
      } catch (err: any) {
        console.error("Failed to fetch partnership agreement:", err);
        setError(err.response?.data?.message || err.message || "Failed to load partnership agreement");
        toast.error("Failed to load partnership agreement");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBack = () => router.push("/dashboard/document-generator");

  const handleDownload = async (format: "docx" | "pdf") => {
    if (!document) return;
    try {
      if (format === "pdf") await downloadAsPDF();
      else await downloadAsDOCX();
    } catch (error) {
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${document?.party1_name || "partnership-agreement"}.pdf`);
    } catch (error) {
  ...
  const downloadAsDOCX = async () => {
    ...
    try {
        ...
        sections: [
          {
            properties: {},
            children: [
              new Paragraph({ text: "Partnership Agreement", heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER }),
              new Paragraph({ children: [new TextRun({ text: `Party 1: ${document.party1_name}`, bold: true })] }),
              new Paragraph({ children: [new TextRun({ text: `Party 2: ${document.party2_name}`, bold: true })] }),
      ...
      link.href = url;
      link.download = `${document?.party1_name || "partnership-agreement"}.docx`;
  ...
  const handleDelete = async () => {
    if (!document) return;
    setIsDeleting(true);
    showLoader("Deleting partnership agreement...");
    try {
      await deletePartnershipAgreement(document.id);
      setDownloading(null);
  ...
  const handleSave = async () => {
    if (!document) return;
    setIsUpdating(true);
    showLoader("Updating partnership agreement...");
    try {
      const updated = await updatePartnershipAgreement(document.id, { ai_generated_content: editedContent });
      setDocument(updated);
      setIsEditing(false);
      setDownloading(null);
      toast.success("Partnership agreement updated successfully!");
    } catch (error: any) {
      setDownloading(null);
      console.error("Failed to update partnership agreement:", error);
      toast.error(error.response?.data?.message || error.message || "Failed to update partnership agreement");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="w-full">
        <DesktopHeader />
        <MobileHeader title="Partnership Agreement" showBorder className="mb-2" />
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-center min-h-[calc(100svh-72px)] lg:min-h-[calc(100svh-56px)]">
          <div className="text-center">
            <div className="mb-4 text-red-500">{error || "Partnership agreement not found"}</div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>Are you sure you want to delete this partnership agreement? This action cannot be undone.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
